...
(async()=>{
 ...
 try{
  ...
  await updater.locator('#projectMenu').click();await updater.locator('#projectList .project-row').first().getByRole('button',{name:'開啟',exact:true}).click();await updater.waitForFunction(()=>!document.querySelector('#export').disabled);
  ...
  assert.ok((await updater.locator('.badge').textContent()).includes(currentVersion));await updater.locator('#projectMenu').click();await updater.locator('#projectList .project-row').first().getByRole('button',{name:'開啟',exact:true}).click();await updater.waitForFunction(()=>!document.querySelector('#export').disabled);pass('updated app and recovered project reopen offline');
